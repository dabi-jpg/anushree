...
export const CardSwiper: React.FC<CardSwiperProps> = ({
  ...
  useEffect(() => {
    if (onActiveIndexChange && cardOrder.length > 0) {
      onActiveIndexChange(cardOrder[0]);
    }
  }, [cardOrder, onActiveIndexChange]);
  ...
  const getCards = useCallback((): HTMLElement[] => {
    ...
    return [...cardStackRef.current.querySelectorAll('.swipe-card-item')] as HTMLElement[];
  }, []);
  ...
  return (
    <section
      ...
      style={{
        ...
        '--card-z-offset': '12px',
        '--card-y-offset': '7px',
        '--card-max-z-index': children.length.toString(),
    ...
    >
      {cardOrder.map((originalIndex, displayIndex) => (
        <article
          key={originalIndex}
          className="swipe-card-item absolute cursor-grab active:cursor-grabbing
                     place-self-center border-2 border-teal-accent rounded-none
                     shadow-retro-pink overflow-hidden will-change-transform bg-white"
          style={{
            '--i': (displayIndex + 1).toString(),
            zIndex: children.length - displayIndex,
            width: cardWidth,
            height: cardHeight,
